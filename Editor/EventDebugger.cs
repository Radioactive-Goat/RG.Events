...
namespace RG.Events
{
    public class EventDebugger : EditorWindow
    {
        ...
        private void BuildAllLists()
        {
            ...
            {
                _subscriberList.itemsSource = EventSystem.Instance.Events.ToList()[_eventList.selectedIndex].Value.StringizedSubscribers;
                _subscriberList.makeItem = BuildListLabel;
                _subscriberList.bindItem = (e, i) => (e as Label).text = EventSystem.Instance.Events.ToList()[_eventList.selectedIndex].Value.StringizedSubscribers[i];
